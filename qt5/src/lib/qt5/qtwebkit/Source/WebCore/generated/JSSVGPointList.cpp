...
void JSSVGPointList::finishCreation(JSGlobalData& globalData)
{
...
{
    return JSSVGPointListPrototype::create(exec->globalData(), globalObject, JSSVGPointListPrototype::createStructure(globalObject->globalData(), globalObject, globalObject->objectPrototype()));
}
...
EncodedJSValue JSC_HOST_CALL jsSVGPointListPrototypeFunctionInitialize(ExecState* exec)
{
    ...
    if (exec->argumentCount() > 0 && !exec->argument(0).isUndefinedOrNull() && !exec->argument(0).inherits(&JSSVGPoint::s_info))
        return throwVMTypeError(exec);
    SVGPropertyTearOff<FloatPoint>* item(toSVGPoint(MAYBE_MISSING_PARAMETER(exec, 0, DefaultIsUndefined)));
