...
bool JSInjectedScriptHostConstructor::getOwnPropertySlot(JSCell* cell, ExecState* exec, PropertyName propertyName, PropertySlot& slot)
{
    return getStaticValueSlot<JSInjectedScriptHostConstructor, JSDOMWrapper>(exec, &JSInjectedScriptHostConstructorTable, jsCast<JSInjectedScriptHostConstructor*>(cell), propertyName, slot);
}

bool JSInjectedScriptHostConstructor::getOwnPropertyDescriptor(JSObject* object, ExecState* exec, PropertyName propertyName, PropertyDescriptor& descriptor)
{
    return getStaticValueDescriptor<JSInjectedScriptHostConstructor, JSDOMWrapper>(exec, &JSInjectedScriptHostConstructorTable, jsCast<JSInjectedScriptHostConstructor*>(object), propertyName, descriptor);
}
...
static const HashTableValue JSInjectedScriptHostPrototypeTableValues[] =
{
    ...
    { "evaluate", DontDelete | JSC::Function, (intptr_t)static_cast<NativeFunction>(jsInjectedScriptHostPrototypeFunctionEvaluate), (intptr_t)1, NoIntrinsic },
    { 0, 0, 0, 0, NoIntrinsic }
};
...
JSValue jsInjectedScriptHostConstructor(ExecState* exec, JSValue slotBase, PropertyName)
{
    JSInjectedScriptHost* domObject = jsCast<JSInjectedScriptHost*>(asObject(slotBase));
...
{
    return getDOMConstructor<JSInjectedScriptHostConstructor>(exec, jsCast<JSDOMGlobalObject*>(globalObject));
}
...
EncodedJSValue JSC_HOST_CALL jsInjectedScriptHostPrototypeFunctionEvaluate(ExecState* exec)
{
    JSValue thisValue = exec->hostThisValue();
    if (!thisValue.inherits(&JSInjectedScriptHost::s_info))
        return throwVMTypeError(exec);
