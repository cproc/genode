...
{
    return getHashTableForGlobalData(exec->globalData(), &JSWebSocketConstructorTable);
}
...
void setJSWebSocketBinaryType(ExecState* exec, JSObject* thisObject, JSValue value)
{
    ...
    JSWebSocket* castedThis = jsCast<JSWebSocket*>(thisObject);
    WebSocket* impl = static_cast<WebSocket*>(castedThis->impl());
    impl->setBinaryType(value.isEmpty() ? String() : value.toString(exec)->value(exec));
}
...
static EncodedJSValue JSC_HOST_CALL jsWebSocketPrototypeFunctionSend1(ExecState* exec)
{
    ...
    ExceptionCode ec = 0;
    ArrayBuffer* data(toArrayBuffer(MAYBE_MISSING_PARAMETER(exec, 0, DefaultIsUndefined)));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());

    JSC::JSValue result = jsBoolean(impl->send(data, ec));
...
static EncodedJSValue JSC_HOST_CALL jsWebSocketPrototypeFunctionSend2(ExecState* exec)
{
    ...
    if (exec->argumentCount() < 1)
        return throwVMError(exec, createNotEnoughArgumentsError(exec));
    ExceptionCode ec = 0;
    ArrayBufferView* data(toArrayBufferView(MAYBE_MISSING_PARAMETER(exec, 0, DefaultIsUndefined)));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
...
EncodedJSValue JSC_HOST_CALL jsWebSocketPrototypeFunctionClose(ExecState* exec)
{
    ...
    const String& reason(MAYBE_MISSING_PARAMETER(exec, 1, DefaultIsUndefined).isEmpty() ? String() : MAYBE_MISSING_PARAMETER(exec, 1, DefaultIsUndefined).toString(exec)->value(exec));
    if (exec->hadException())
        return JSValue::encode(jsUndefined());
