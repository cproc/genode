...
static void initThreadPipeFD(int fd)
{
    ...
    if (ret == -1)
        perror("QEventDispatcherGenodePrivate: Unable to init thread pipe");

    int flags = fcntl(fd, F_GETFL);
    if (flags == -1)
        perror("QEventDispatcherGenodePrivate: Unable to get flags on thread pipe");

    ret = fcntl(fd, F_SETFL, flags | O_NONBLOCK);
    if (ret == -1)
        perror("QEventDispatcherGenodePrivate: Unable to set flags on thread pipe");
}
#endif

QEventDispatcherGenodePrivate::QEventDispatcherGenodePrivate()
{
   ...
   // do nothing.
    ...
    if (socketpair(AF_INET, SOCK_STREAM, 0, thread_pipe) == -1) {
        perror("QEventDispatcherGenodePrivate(): Unable to create socket pair");
        pipefail = true;
    } else {
    ...
    if (pipeDevCreate(name, 128 /*maxMsg*/, 1 /*maxLength*/) != OK) {
        perror("QEventDispatcherGenodePrivate(): Unable to create thread pipe device");
        pipefail = true;
    } else {
        if ((thread_pipe[0] = open(name, O_RDWR, 0)) < 0) {
            perror("QEventDispatcherGenodePrivate(): Unable to create thread pipe");
            pipefail = true;
        } else {
            initThreadPipeFD(thread_pipe[0]);
    ...
    if (pipefail)
        qFatal("QEventDispatcherGenodePrivate(): Can not continue without a thread pipe");
...
int QEventDispatcherGenodePrivate::doSelect(QEventLoop::ProcessEventsFlags flags, timeval *timeout)
{
    Q_Q(QEventDispatcherGenode);

    // needed in QEventDispatcherGenode::select()
...
int QEventDispatcherGenodePrivate::initThreadWakeUp()
{
    FD_SET(thread_pipe[0], &sn_vec[0].select_fds);
    return thread_pipe[0];
}

int QEventDispatcherGenodePrivate::processThreadWakeUp(int nsel)
{
    if (nsel > 0 && FD_ISSET(thread_pipe[0], &sn_vec[0].select_fds)) {
        ...
        if (!wakeUps.testAndSetRelease(1, 0)) {
            ...
            qWarning("QEventDispatcherGenode: internal error, wakeUps.testAndSetRelease(1, 0) failed!");
        }
        return 1;
    }
    return 0;
...
QEventDispatcherGenode::QEventDispatcherGenode(QObject *parent)
    : QAbstractEventDispatcher(*new QEventDispatcherGenodePrivate, parent)
{ }

QEventDispatcherGenode::QEventDispatcherGenode(QEventDispatcherGenodePrivate &dd, QObject *parent)
    : QAbstractEventDispatcher(dd, parent)
{ }

QEventDispatcherGenode::~QEventDispatcherGenode()
{
...
/*!
    \internal
*/
void QEventDispatcherGenode::registerTimer(int timerId, int interval, Qt::TimerType timerType, QObject *obj)
{
#ifndef QT_NO_DEBUG
    if (timerId < 1 || interval < 0 || !obj) {
        qWarning("QEventDispatcherGenode::registerTimer: invalid arguments");
        return;
...
/*!
    \internal
*/
bool QEventDispatcherGenode::unregisterTimer(int timerId)
{
#ifndef QT_NO_DEBUG
    if (timerId < 1) {
        qWarning("QEventDispatcherGenode::unregisterTimer: invalid argument");
        return false;
...
/*!
    \internal
*/
bool QEventDispatcherGenode::unregisterTimers(QObject *object)
{
#ifndef QT_NO_DEBUG
    if (!object) {
        qWarning("QEventDispatcherGenode::unregisterTimers: invalid argument");
        return false;
...
QList<QEventDispatcherGenode::TimerInfo>
QEventDispatcherGenode::registeredTimers(QObject *object) const
{
    if (!object) {
        qWarning("QEventDispatcherGenode:registeredTimers: invalid argument");
        return QList<TimerInfo>();
    }

    Q_D(const QEventDispatcherGenode);
...
/*****************************************************************************
 QEventDispatcher implementations for Genode
 *****************************************************************************/

void QEventDispatcherGenode::registerSocketNotifier(QSocketNotifier *notifier)
{
    Q_ASSERT(notifier);
...
void QEventDispatcherGenode::unregisterSocketNotifier(QSocketNotifier *notifier)
{
    Q_ASSERT(notifier);
    int sockfd = notifier->socket();
    int type = notifier->type();
...
void QEventDispatcherGenode::setSocketNotifierPending(QSocketNotifier *notifier)
{
    Q_ASSERT(notifier);
    int sockfd = notifier->socket();
    int type = notifier->type();
...
int QEventDispatcherGenode::activateTimers()
{
...
int QEventDispatcherGenode::activateSocketNotifiers()
{
    Q_D(QEventDispatcherGenode);
    if (d->sn_pending_list.isEmpty())
        return 0;
...
bool QEventDispatcherGenode::processEvents(QEventLoop::ProcessEventsFlags flags)
{
    Q_D(QEventDispatcherGenode);
    d->interrupt = false;
...
bool QEventDispatcherGenode::hasPendingEvents()
{
    extern uint qGlobalPostedEventsCount(); // from qapplication.cpp
    return qGlobalPostedEventsCount();
}

int QEventDispatcherGenode::remainingTime(int timerId)
{
#ifndef QT_NO_DEBUG
    if (timerId < 1) {
        qWarning("QEventDispatcherGenode::remainingTime: invalid argument");
        return -1;
    }
#endif
...
void QEventDispatcherGenode::interrupt()
{
    Q_D(QEventDispatcherGenode);
    d->interrupt = true;
    wakeUp();
...
void QEventDispatcherGenode::flush()
{ }
