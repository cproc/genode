...
{
	//lx_printf("hid_match_one_id()\n");
	//lx_printf("hid_match_one_id(): hdev->bus: %u\n", hdev->bus);
	bool result = (id->bus == HID_BUS_ANY || id->bus == hdev->bus) &&
...
const struct hid_device_id *hid_match_id(const struct hid_device *hdev,
		const struct hid_device_id *id)
{
//lx_printf("hid_match_id(): ret: %p\n", __builtin_return_address(0));
//lx_printf("hid_match_id(): hdev->bus: %u\n", hdev->bus);

	for (; id->bus; id++)
		if (hid_match_one_id(hdev, id)) {
//lx_printf("hid_match_id(): match\n");
			return id;
		}

//lx_printf("hid_match_id(): no match\n");
	return NULL;
}
...
//lx_printf("hid_connect(): finished\n");
	return 0;
}
					     ...
					     struct hid_driver *hdrv)
{
	...
	spin_lock(&hdrv->dyn_lock);
//lx_printf("hid_match_device(): ret: %p\n", __builtin_return_address(0));
	list_for_each_entry(dynid, &hdrv->dyn_list, list) {
		if (hid_match_one_id(hdev, &dynid->id)) {
//lx_printf("hid_match_device(): finished 1\n");
			spin_unlock(&hdrv->dyn_lock);
			return &dynid->id;
		}
	}
	...
	const struct hid_device_id *result = hid_match_id(hdev, hdrv->id_table);
//lx_printf("hid_match_device(): finished 2\n");
	return result;
}
...
{
lx_printf("hid_bus_match(): dev: %p\n", dev);
	struct hid_driver *hdrv = to_hid_driver(drv);
	struct hid_device *hdev = to_hid_device(dev);

lx_printf("hid_bus_match(): hdev: %p\n", hdev);
lx_printf("hid_bus_match(): hdev->bus: %u\n", hdev->bus);

	int result = hid_match_device(hdev, hdrv) != NULL;
lx_printf("hid_bus_match(): %d\n", result);
	return result;
}
...
lx_printf("hid_device_probe(): dev: %p, hdev: %p\n", dev, hdev);
...
lx_printf("hid_device_probe() finished: %d\n", ret);
	return ret;
}
