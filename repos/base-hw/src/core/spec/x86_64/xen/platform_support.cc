...
#define CPUID(command, result) \
	asm volatile("CPUID" : "=b" (result.r[0]), "=c" (result.r[1]), "=d" (result.r[2]) : "a" (command));

void Platform::_init_additional()
{
	/* Xen-specific initialization */
	...
	Genode::log("CPUID: ", Cstring(cpu.string));

	if (strcmp(cpu.string, "XenVMMXenVMM", 12) == 0)
		log("Running as Xen HVM guest");
	else
		log("Not running as Xen HVM guest");
