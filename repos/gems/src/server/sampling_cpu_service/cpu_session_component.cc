...
Thread_capability
Sampling_cpu_service::Cpu_session_component::create_thread(size_t weight,
                                                           Name const &name,
                                                           addr_t utcb)
{
	Thread_capability thread_cap = _parent_cpu_session.create_thread(weight, name.string(), utcb);

	Thread_data *thread_data = new (_md_alloc) Thread_data(*this, thread_cap, name.string());

	_thread_list.insert(thread_data);
...
void Sampling_cpu_service::Cpu_session_component::kill_thread(Thread_capability thread_cap)
{
	...
	for (Thread_data *thread_data = _thread_list.first();
	     thread_data;
	     thread_data = thread_data->next()) {
		if (thread_data->thread_cap() == thread_cap) {
			_thread_list.remove(thread_data);
...
int Sampling_cpu_service::Cpu_session_component::set_pager(Thread_capability thread_cap,
...
Sampling_cpu_service::Cpu_session_component::Cpu_session_component(Allocator *md_alloc,
...
int Sampling_cpu_service::Cpu_session_component::ref_account(Cpu_session_capability) { return -1; }

int Sampling_cpu_service::Cpu_session_component::transfer_quota(Cpu_session_capability, size_t) { return -1; }

Cpu_session::Quota
Sampling_cpu_service::Cpu_session_component::quota() { return Quota(); }
