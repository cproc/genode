...
void Decorator::Main::handle_pointer_update(unsigned)
{
	...
	Decorator::Point const position = point_attribute(pointer_node);

	Window_base::Hover const new_hover = window_stack.hover(position);
		...
		Reporter::Xml_generator xml(hover_reporter, [&] ()
		{
			if (hover.window_id > 0) {
				...
				xml.node("window", [&] () {
					...
					if (hover.upper_sizer) xml.node("upper_sizer");
					if (hover.lower_sizer) xml.node("lower_sizer");
