                                           ...
                                           public List<View_component>::Element
{
	...
	public:
		 ...
		 */
		Point anchor_position() const
		{
			if (is_top_level()) {
				return _anchor_position;
			} else {
				return _parent->anchor_position();
			}
		}
		...
		void update()
		{
			if (!_real_view.valid() && _parent && _parent->real_view().valid()) {
				...
				_real_view = _real_nitpicker.create_view(_parent->real_view());

				View_client(_real_view).viewport(_geometry.x1(), _geometry.y1(), _geometry.w(), _geometry.h(),
				            _buffer_offset.x(), _buffer_offset.y(), true);
		             ...
		             int buf_x, int buf_y, bool redraw) override
		{
			...
			if (is_top_level()) {
				...
				if (!_real_view.valid() && _parent->real_view().valid()) {
					_real_view = _real_nitpicker.create_view(_parent->real_view());
					View_client(_real_view).stack(View_capability(), true, true);
				}
		...
		Point _input_origin() const
		{
			for (View_component const *v = _views.first(); v; v = v->next()) {
				...
				if (v->is_top_level()) {
					...
					return Point(0, 0) - v->anchor_position();
				}
			}
