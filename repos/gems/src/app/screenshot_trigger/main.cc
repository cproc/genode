...
struct Screenshot_trigger::Main
{
	...
	unsigned const _size     { _config.xml().attribute_value("size", 50u) };
	Point    const _position { 0, 0 };
	Area     const _area     { _size, _size };
	...
	Gui_buffer _gui_buffer { _gui, _area, _env.ram(), _env.rm() };
