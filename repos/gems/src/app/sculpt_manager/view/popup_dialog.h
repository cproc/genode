...
struct Sculpt::Popup_dialog : Deprecated_dialog
{
	...
	Constructible<Debug_dialog>    _debugging { };
	...
	bool _debugging_dialog_selected() const
	{
		return _route_selected("debugging");
	}
	...
	Hover_result hover(Xml_node hover) override
	{
		...
		if (_debugging.constructed() &&
		    hover_result == Deprecated_dialog::Hover_result::UNMODIFIED)
			hover_result = _debugging->match_sub_dialog(hover, "frame", "vbox", "frame", "vbox");
	...
	void apply_blueprint(Component &construction, Xml_node blueprint)
	{
		...
		_debugging.construct(construction.monitor,
		                     construction.wait,
		                     construction.wx);
