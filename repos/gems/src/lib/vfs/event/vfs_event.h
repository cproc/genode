		...
		class Event_vfs_handle : public Single_vfs_handle
		{
			...
			public:
				...
				Write_result write(char const *src, Vfs::file_size count,
				                   Vfs::file_size &out_count) override
				{
					Genode::log("Event_vfs_handle::write(): count: ", count);

					out_count = 0;

					while (count - out_count >= sizeof(Genode::uint32_t)) {

						Genode::uint32_t value = (Genode::uint32_t)(src[out_count]);

						Genode::log("value: ", Genode::Hex(value));

						_event.with_batch([&] (Event::Session_client::Batch &batch) {
							batch.submit(Input::Press_char{Input::KEY_UNKNOWN,
							                               Genode::Codepoint{value}});
							batch.submit(Input::Release{Input::KEY_UNKNOWN});
						});

						out_count += sizeof(Genode::uint32_t);
					}

					Genode::log("Event_vfs_handle::write(): finished");
