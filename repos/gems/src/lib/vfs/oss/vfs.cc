...
struct Vfs::Oss_file_system::Context_factory : File_system_factory
{
	...
	Context_factory(Vfs::Env &env, Xml_node const &config)
	:
		_env     { env },
                                                  ...
                                                  public  Vfs::Dir_file_system
{
	private:
		...
		static Config _config(Name const &name)
		{
			...
			Genode::Xml_generator::generate({ buf, sizeof(buf) }, "dir",
			                                [&] (Genode::Xml_generator &xml) {
	...
	public:
		...
		Context_file_system(Vfs::Env &vfs_env, Genode::Xml_node const &config,
...
:
	...
	Vfs::Dir_file_system { vfs_env,
	                       Xml_node(_config(Name(id())).string()),
	                       *this }
{ }
	...
	public:
		...
		:
			Single_file_system { Node_type::CONTINUOUS_FILE, name.string(),
			                     Node_rwx::ro(), Genode::Xml_node("<oss_data/>") },
			_ep          { ep },
			_vfs_env     { vfs_env },
			_config      { config },
                                                   ...
                                                   public  Vfs::Dir_file_system
{
	...
	public:
		...
		Compound_file_system(Vfs::Env &vfs_env, Genode::Xml_node const &config)
		:
			Local_factory { vfs_env, config },
			Vfs::Dir_file_system { vfs_env,
			                       Xml_node(_config(Local_factory::name(config)).string()),
			                       *this }
		{ }
...
extern "C" Vfs::File_system_factory *vfs_file_system_factory(void)
{
	struct Factory : Vfs::File_system_factory
	{
		...
		{
			static Genode::Buffered_xml buffered_config(env.alloc(), config);
			return new (env.alloc())
				Vfs::Oss_file_system::Compound_file_system(env, buffered_config.xml);
