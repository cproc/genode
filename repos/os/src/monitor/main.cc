...
Genode::log("command: ", Genode::Cstring(buffer));
		...
		void thread_stopped(Monitored_thread &)
		{
...
Genode::log("Gdb_stub(): calling _handle_terminal_read_avail()");
	...
	{
		Local_cpu_session &session =
			*new (_heap) Local_cpu_session(_env, label, _heap, *this);
	...
	void set_initial_breakpoint(Monitored_thread &thread, addr_t addr,
	                            char original_instruction[]) override
	{
		...
		_inferiors.for_each<Inferior_pd &>([&] (Inferior_pd &inferior) {
			inferior._threads.for_each<Monitored_thread &>([&] (Monitored_thread &t) {
	...
	void remove_initial_breakpoint(Monitored_thread &thread, addr_t addr,
	...
	void thread_stopped(Monitored_thread &thread) override
	{
		if (!_gdb_stub.constructed()) {
		...
		_gdb_stub->thread_stopped(thread);
