...
//Genode::log("parse_registers(): i: ", i);
...
//Genode::log("parse_registers(): ", Genode::Cstring(null_terminated));
				addr_t value = 0;
				ascii_to_unsigned(null_terminated, value, 16);
//Genode::log("parse_registers(): ", Genode::Hex(value));
				*values_64bit[i] = big_endian_to_host(value);
//Genode::log("parse_registers(): ", Genode::Hex(*values_64bit[i]));
			});
		});
	}
...
//Genode::log("parse_registers(): ", Genode::Hex(*values_32bit[i]));
			});
		});
	}
}
