...
Genode::log("complete(): ", &iface, ", ret: ", __builtin_return_address(0));
		...
		if (!p.read_transfer()) {
			...
			if (req.pending) {
				...
				if (!request_executed) {
					...
					Genode::log("request_executed 1: ", request_executed);
				} else {
			...
			iface.release(p);
Genode::log("complete() finished 1");
			return;
		}
		...
		do {
			...
			Genode::log("request_executed 2: ", request_executed);
			ack_pending_request(); /* calls cbw() */
		} while (0);

		iface.release(p);
Genode::log("complete() finished 2");
	}
