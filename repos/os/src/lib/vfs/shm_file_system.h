...
class Vfs_shm::Node : private Genode::Avl_node<Node>, private Genode::Lock
{
	private:
	...
	public:
		...
		{
			name(node_name);
			Genode::warning(this, ": Node(): ", Genode::Cstring(node_name));
		}

		virtual ~Node()
		{
			Genode::warning(this, ": ~Node()");
			//wait_for_continue();
		}
		...
		struct Guard
		{
			...
			Guard(Node *guard_node) : node(*guard_node)
			{
				//Genode::warning("Guard()");
				node.lock();
				//Genode::warning("Guard(): locked");
			}
...
class Vfs::Shm_file_system : public Vfs::File_system
{
	...
	public:
		...
		{
Genode::warning("Vfs::Shm_file_system::open(): path: ", Genode::Cstring(path),
			...
			if (create) {
				...
				parent->adopt(file);
				parent->notify();
Genode::warning("Vfs::Shm_file_system::open(): new file created: ", file);
			} else {
				Node *node = lookup(path);
...
Genode::warning("Vfs::Shm_file_system::open(): file found: ", file);
			}

			try {
				*handle = new (alloc) Io_handle(*this, alloc, mode, *file);
Genode::warning("Vfs::Shm_file_system::open(): *handle: ", *handle);
				return OPEN_OK;
		...
		{
			using namespace Vfs_shm;
Genode::warning("Vfs::Shm_file_system::dataspace(): ", Genode::Cstring(path));
			Ram_dataspace_capability ds_cap;
		                   ...
		                   Allocator        &alloc) override
		{
			...
			try {
				Vfs_shm::Watch_handle *watch_handle = new(alloc)
					Vfs_shm::Watch_handle(*this, alloc, *node);
		...
		{
			Vfs_shm::Watch_handle *watch_handle =
				static_cast<Vfs_shm::Watch_handle *>(vfs_handle);
		                          ...
		                          file_size &out_count) override
		{
			...
			Vfs_shm::Io_handle const *handle =
				dynamic_cast<Vfs_shm::Io_handle *>(vfs_handle);
Genode::warning("Vfs::Shm_file_system::complete_read(): handle: ", handle);
Genode::warning("Vfs::Shm_file_system::complete_read(): node: ", &handle->node);
			Vfs_shm::Node::Guard guard(&handle->node);

Genode::warning("Vfs::Shm_file_system::complete_read(): calling node.complete_read()");
			Read_result res = handle->node.complete_read(dst, count, handle->seek(), out_count);
Genode::warning("Vfs::Shm_file_system::complete_read(): node.complete_read() returned");
		...
		{
Genode::warning("Vfs::Shm_file_system::ftruncate(): len: ", len);
			if ((vfs_handle->status_flags() & OPEN_MODE_ACCMODE) ==  OPEN_MODE_RDONLY)
...
Genode::warning("Vfs::Shm_file_system::ftruncate() finished");
			return FTRUNCATE_OK;
		}
