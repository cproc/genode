...
class Decorator::Window_base : public Window_list::Element
{
	...
	private:
		...
		Nitpicker::View_capability _neighbor;
		...
		struct Nitpicker_view : Nitpicker::View_client
		{
			...
			Nitpicker_view(Nitpicker::Session &nitpicker, unsigned id = 0)
			:
			...
			void stack(Nitpicker::View_capability neighbor)
			{
				if (neighbor.valid())
					Nitpicker::View_client::stack(neighbor, false, false);
				else
					Nitpicker::View_client::stack(neighbor, true, false);
			}
	...
	public:
		...
		Nitpicker::View_capability anchor_view() { return _content_view; }
		...
		virtual void update_nitpicker_views()
		{
			...
			if (!_nitpicker_stacking_up_to_date) {
				...
				_left_view.stack(_top_view);
				_right_view.stack(_left_view);
				_bottom_view.stack(_right_view);
				_content_view.stack(_bottom_view);
