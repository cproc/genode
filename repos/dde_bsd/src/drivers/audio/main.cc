...
if (/*(count == 0) &&*/ (data[i] != 0)) {
	play_count++;
	Genode::trace("audio_drv play: ", play_count, ", i: ", data[i],
	              ", f: ", p_left->content()[i / 2],
	              ", ", (Genode::Trace::timestamp() - record_ts) / 2496000, " ms");
//	Genode::log("audio_drv play: ", play_count, ", i: ", data[i],
//	            ", f: ", p_left->content()[i / 2]);
	count++;
}
#endif
...
static int count = 0;
if ((count == 0) && (sample != 0)) {
Genode::trace("audio_drv record: ", i, ", i: ", data[i],
              ", f: ", p->content()[i/2]);
//Genode::log("audio_drv record: ", i, ", i: ", data[i],
//            ", f: ", p->content()[i/2]);
count++;
}
#endif
...
#else
			static int count = 0;
//if (count < 2500) { Genode::trace("count: ", count); }
			for (int i = 0; i < Audio_in::PERIOD; i++) {
				if ((count >= 0) && (play_count == record_count) && (i == 2)) {
					int16_t sample = 1024;
					p->content()[i] = (float)sample / 32768.0f;
					record_count++;
					record_ts = Genode::Trace::timestamp();
Genode::trace("audio_drv record: ", record_count, ", i: ", sample,
              ", f: ", p->content()[i]);
//Genode::log("audio_drv record: ", record_count, ", i: ", sample,
//            ", f: ", p->content()[i]);
				} else {
					p->content()[i] = 0;
				}
