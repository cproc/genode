			...
			public:
		  	  	  ...
		  	  	  _ep(ep)
				{
					if (_ep != 0) {
						...
						_usb_connection.endpoint_descriptor(0, 0, _ep, &_endpoint_descriptor);

						Genode::log("*** EP: ", _ep, ", address: ", Genode::Hex(_endpoint_descriptor.address),
				...
				Write_result write(file_size count, file_size &out_count)
				{
					...
					switch (_ep_type) {
						...
						case Usb::ENDPOINT_CONTROL: {
							...
							file_size bytes_consumed =
								write_callback((char*)&setup, sizeof(setup), count == sizeof(setup) ?
								                              	  	  Callback::COMPLETE :
								                              	  	  Callback::PARTIAL);

							Genode::log("count: ", count, ", bytes_consumed: ", bytes_consumed);
