...
class Vfs::Usb_file_system : public File_system
{
	private:
			...
			public:
		  	  	  ...
		  	  	  _ep(ep)
				{
					if (_ep != 0) {
						...
						Usb::Interface_descriptor interface_descriptor;
						Usb::Endpoint_descriptor  endpoint_descriptor;

						_usb_connection.interface_descriptor(0, 0, &interface_descriptor);

						for (unsigned int i = 0; i < interface_descriptor.num_endpoints; i++) {
							_usb_connection.endpoint_descriptor(0, 0, i, &endpoint_descriptor);
							if ((endpoint_descriptor.address & 0x0f) == _ep)
								break;
						}

						Genode::log("Ep_file_handle(): _ep: ", _ep, ", address: ", Genode::Hex(endpoint_descriptor.address),
						    		", attributes: ", Genode::Hex(endpoint_descriptor.attributes));

						_ep_type =
							static_cast<Usb::Endpoint_type>(endpoint_descriptor.attributes & 0x3);

						_ep_direction =
							static_cast<Usb::Endpoint_direction>(endpoint_descriptor.address & 0x80);
				...
				Read_result read(file_size count, file_size &out_count)
				{
					...
					switch (_acked_packet.type) {
						...
						case Usb::Packet_descriptor::CTRL: {
							...
							Genode::log(__PRETTY_FUNCTION__, ": CTRL: ", _acked_packet.control.actual_size);
						...
						case Usb::Packet_descriptor::IRQ:
							Genode::log(__PRETTY_FUNCTION__, ": BULK or IRQ");

							if (_ep_direction == Usb::ENDPOINT_IN) {
		...
		bool _ep_directory(const char *path)
		{
			return (strlen(path) == strlen("/ep")) &&
			       (strcmp(path, "/ep") == 0);
		}
		...
		{
			Genode::Path<8> genode_path(path);

			if (!genode_path.strip_prefix("/ep"))
				return false;

			unsigned int ep = 0;
			if (Genode::ascii_to(&genode_path.base()[1], ep) == 0)
				return false;
		                 ...
		                 Allocator   &alloc) override
		{
			if (_state_file(path))
				*out_handle = new (alloc) State_file_handle(*this, *this, alloc, 0,
				...
				Genode::Path<8> genode_path(path);
				genode_path.strip_prefix("/ep");
				unsigned int ep = 0;
				Genode::ascii_to(&genode_path.base()[1], ep);
