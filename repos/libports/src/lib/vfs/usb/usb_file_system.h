...
class Vfs::Usb_file_system : public File_system
{
	private:
		...
		{
			return (strlen(path) == (strlen("state") + 1)) &&
			       (strcmp(&path[1], "state") == 0);
		}
	...
	public:
		...
		Read_result read(Vfs_handle *vfs_handle, file_size count,
	                 	 file_size &out_count) override
		{
			if (_state == PLUGGED) /* XXX plugged */
				out_count = vfs_handle->read_callback("unplugged", count, Callback::COMPLETE);
			else
				out_count = vfs_handle->read_callback("unplugged", count, Callback::COMPLETE);
