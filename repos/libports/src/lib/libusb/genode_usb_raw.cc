...
struct Usb_device
{
	...
	Usb_device()
	{
		...
		int ep0_fd = open("/usb/ep/0", O_RDWR);
...
Genode::log(__PRETTY_FUNCTION__);
	...
	if (ep_fd[ep] == -1) {
		...
		char ep_filename[Genode::strlen("/usb/ep/xx") + 1];
	...
	switch (transfer->type) {
		...
		case LIBUSB_TRANSFER_TYPE_BULK_STREAM:
		case LIBUSB_TRANSFER_TYPE_INTERRUPT: {
			Genode::log("bulk or interrupt transfer for ep ", ep, ", fd: ", ep_fd[ep]);
...
Genode::log(__PRETTY_FUNCTION__, ": CONTROL");
					...
					if ((setup->bmRequestType & LIBUSB_ENDPOINT_DIR_MASK) ==
					    LIBUSB_ENDPOINT_IN) {
					    ssize_t res = read(pollfd->fd, transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE,
					                       setup->wLength);
						...
						char dummy;
						ssize_t res = read(pollfd->fd, &dummy, sizeof(dummy));
					    Genode::log("received ", res, " bytes");

					    itransfer->transferred = setup->wLength;
					}
				...
				case LIBUSB_TRANSFER_TYPE_BULK_STREAM:
				case LIBUSB_TRANSFER_TYPE_INTERRUPT: {
Genode::log(__PRETTY_FUNCTION__, ": BULK or INTERRUPT");

					if (IS_XFERIN(transfer)) {

						ssize_t res = read(pollfd->fd, transfer->buffer, transfer->length);
					    Genode::log("received ", res, " bytes");
					    itransfer->transferred = res;
					    ...
					    itransfer->transferred = transfer->length;
