...

{
	...
	auto fn = [&] {
		...
		for (int poll_fd_index = 0; poll_fd_index < nfds; poll_fd_index++) {

			bool fd_ready = false;

			if (fds[poll_fd_index].events & POLLIN) {

				try {
					Socket_fs::Context *context =
						static_cast<Socket_fs::Context *>(fds[poll_fd_index].fdo->context);

					if (context->read_ready()) {
						fds[poll_fd_index].revents |= POLLIN;
						fd_ready = true;
					}
				} catch (Socket_fs::Context::Inaccessible) { }
			}

			if (fds[poll_fd_index].events & POLLOUT) {
				try {
					Socket_fs::Context *context =
						static_cast<Socket_fs::Context *>(fds[poll_fd_index].fdo->context);

					if (context->write_ready()) {
						fds[poll_fd_index].revents |= POLLOUT;
						fd_ready = true;
					}
				} catch (Socket_fs::Context::Inaccessible) { }
			}
