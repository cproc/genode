...
{
	int dummy;
Genode::log("init_pthread_support(): ", &dummy);
...
extern "C" {
	...
	pthread_t pthread_self(void)
	{
		...
		Genode::log("pthread_self(): ", Pthread::myself());

		pthread_t pthread_myself = static_cast<pthread_t>(Pthread::myself());

		if (pthread_myself && pthread_registry().contains(*pthread_myself))
