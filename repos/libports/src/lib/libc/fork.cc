                                   ...
                                   File_descriptor_allocator &fd_alloc)
{
	...
	g.node("libc", [&] () {
		...
		_mem_alloc_ptr->for_each_region([&] (Range const range) {
			g.node("mem_alloc", [&] () {
				gen_range_attr(range.start, range.num_bytes); }); });

		_mem_alloc_exec_ptr->for_each_region([&] (Range const range) {
			g.node("mem_alloc_exec", [&] () {
				gen_range_attr(range.start, range.num_bytes); }); });
	});
