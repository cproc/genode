		...
		class Mmap_registry
		{
			public:
				...
				struct Entry : List<Entry>::Element
				{
					...
					Entry(void *start, Libc::File_descriptor *fd)
					: start(start), fd(fd) { }
				};
			...
			public:
				...
				void insert(void *start, Libc::File_descriptor *fd)
				{
					...
					_list.insert(new (&_md_alloc) Entry(start, fd));
				}

				Libc::File_descriptor *lookup_fd_by_addr(void *start) const
				{
					Mutex::Guard guard(_mutex);
