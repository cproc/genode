...
Libc::File_descriptor *Libc::Vfs_plugin::open(char const *path, int flags,
                                              int libc_fd)
{
	unsigned stat_mode = Vfs::Directory_service::STAT_MODE_FILE;
		...
		case Vfs::Directory_service::STAT_ERR_NO_ENTRY:
			if (flags & O_CREAT) {
				if (flags & O_DIRECTORY)
					stat_mode = Vfs::Directory_service::STAT_MODE_DIRECTORY;
				else
					stat_mode = Vfs::Directory_service::STAT_MODE_FILE;
				break;
			}
			errno = ENOENT;
			return nullptr;
	...
	if (stat_mode & Vfs::Directory_service::STAT_MODE_DIRECTORY) {
		...
		switch (_root_dir.opendir(path, (flags & O_CREAT), &handle, _alloc)) {
