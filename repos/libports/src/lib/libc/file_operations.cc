			...
			libc.with_optional_sub_node("mmap", [&] (Node const &mmap) {
				...
				Region_map::Range const range = rm.attach(_mmap_rm()->dataspace(), {
					...
					[&] (Genode::Env::Local_rm::Attachment &a) {
						a.deallocate = false;
						return Region_map::Range((Genode::addr_t)a.ptr, a.num_bytes);
					},
					[&] (Genode::Env::Local_rm::Error e) {
						switch (e) {
						case Genode::Env::Local_rm::Error::OUT_OF_RAM:        result = -2; break;
						case Genode::Env::Local_rm::Error::OUT_OF_CAPS:       result = -4; break;
						case Genode::Env::Local_rm::Error::INVALID_DATASPACE: result = -6; break;
						case Genode::Env::Local_rm::Error::REGION_CONFLICT:   break;
						}
						result = -7;
...
//Genode::log("Libc::init_file_operations(): range: ", Genode::Hex(_mmap_range_start), "-", Genode::Hex(_mmap_range_end));
				...
				if (_mmap_alloc().add_range(range.start, range.num_bytes).failed())
					Genode::error("Libc::init_file_operations(): could not add mmap range");
...
//Genode::log("mmap(): addr: ", addr, ", length: ", length, ", alignment: ", alignment_log2);
		...
		length = align_addr(length, AT_PAGE);
		...
		if (_mmap_use_managed_ds) {
			...
			if (addr) {
				...
				if (((Genode::addr_t)addr < _mmap_range_start) ||
					...
					!_mmap_alloc().alloc_addr(length, Genode::addr_t(addr)).convert<bool>(
			...
			if (!addr) {
				...
				uint8_t align_log2 = Genode::log2(length, 0u);
...

				{
					...
					_mmap_alloc().alloc_aligned(length, { .log2 = align_log2 }).with_result(
						[&] (Range_allocator::Allocation &a) {
							a.deallocate = false;
							addr = a.ptr;
						},
						[&] (Alloc_error) { alloc_ok = false; });
				}
				  ...
				  .writeable  = true
					...
					[&] (Region_map::Attach_error e) {
						...
						case Region_map::Attach_error::INVALID_DATASPACE:
							Genode::error("mmap(): attach() failed: invalid dataspace");
							result = -1;
							break;
						case Region_map::Attach_error::REGION_CONFLICT:
							Genode::error("mmap(): attach() failed: region conflict for address ", addr);
							result = -2;
							break;
						}
						return Region_map::Range { };
...
//Genode::log("mmap() finished: ", addr, " - ", Genode::Hex((addr_t)addr + length - 1));
		...
		void *start = mem_alloc(executable)->alloc(length, { .log2 = alignment_log2 });
