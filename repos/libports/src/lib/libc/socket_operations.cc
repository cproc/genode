...
{
Genode::warning(&libc_fd, ": recvfrom(): fd: ", libc_fd, ", ret: ", __builtin_return_address(0));
	if (*config_socket())
...
{
Genode::warning(&libc_fd, ": recv(): fd: ", libc_fd, ", ret: ", __builtin_return_address(0));
	if (*config_socket())
...
{
Genode::warning(&domain, ": socket(): ret: ", __builtin_return_address(0));
	if (*config_socket()) {
