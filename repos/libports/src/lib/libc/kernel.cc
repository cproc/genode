...
//	_malloc_heap.construct(*_malloc_ram, _env.rm());
	...
	Genode::bzero(&_jemalloc_bss_start, jemalloc_bss_bytes);
}
...
void Libc::Kernel::_clone_state_from_parent()
{
	...
	_with_libc_config([&] (Node const &libc) {
		...
		libc.for_each_sub_node("mem_alloc", [&] (Node const &node) {
			Range const range = range_attr(node);
			new (_heap)
		...
		libc.for_each_sub_node("mem_alloc_exec", [&] (Node const &node) {
			Range const range = range_attr(node);
			new (_heap)
