...
int Libc::Mem_alloc_impl::Dataspace_pool::expand(size_t size)
{
	...
	if (_alloc.add_range(range.start, range.num_bytes).failed())
	...
	return _alloc.alloc_aligned(sizeof(Dataspace), { .log2 = 2 }).convert<int>(
...
void *Libc::Mem_alloc_impl::alloc(size_t size, Align align)
{
	...
	if (_ds_pool.expand(align_addr(request_size, AT_PAGE)) < 0) {
