...
void QNitpickerPlatformWindow::_handle_input(unsigned int)
{
	...
	_input_session.for_each_event([&] (Input::Event const &event) {
		...
		if (event.type() == Input::Event::MOTION ||
			...
			Genode::log("got a CHARACTER event");
			
			Genode::log("code: ", Genode::Hex(event.code()));

			//Input::Event::Utf8 const utf8 = ev->utf8();

			//char utf8_str[4] = { utf8.b0, utf8.b1, utf8.b2, utf8.b3 };
			
			//Genode::log("utf8: ", Genode::Hex(utf8.b0));

			int code = event.code();
			char const *code_str = (char const*)&code;

			qDebug() << "text: " << QString::fromUtf8(code_str);

		    QWindowSystemInterface::handleKeyEvent(0, QEvent::KeyPress, 0, 0, QString::fromUtf8(code_str));
		}
	});
