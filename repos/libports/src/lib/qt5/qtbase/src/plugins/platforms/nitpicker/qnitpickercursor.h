...
class QNitpickerCursor : public QPlatformCursor
{
	...
	public:
		...
		virtual void changeCursor(QCursor *widgetCursor, QWindow *window) override
		{
			...
			_shape_report->width = 32;//cursor.width();
			_shape_report->height = 32;//cursor.height();
Genode::log("width: ", _shape_report->width, ", height: ", _shape_report->height);
			//memcpy(_shape_report->shape, cursor.constBits(), cursor.width() * cursor.height() * 4);
			memset(_shape_report->shape, 0xff, _shape_report->width * _shape_report->height * 4);
Genode::log("submitting report");
