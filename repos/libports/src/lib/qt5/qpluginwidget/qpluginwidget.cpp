...
#if 0
		Nitpicker::View_capability parent_view_cap;
		if (platform_window->window()->transientParent()) {
			qDebug() << "transient parent";
			QNitpickerPlatformWindow *parent_platform_window =
				static_cast<QNitpickerPlatformWindow*>(platform_window->window()->transientParent()->handle());
			parent_view_cap = parent_platform_window->view_cap();
		}
#endif
