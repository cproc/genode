...
void Entrypoint::Signal_proxy_component::signal()
{
	...
	try {
		...
		//if (sig.context()->level() != Signal_context::Level::App)
			//Genode::debug("signal(): dispatching io signal: ", &sig);
		...
		do {
			...
			if (success) {
				...
				retry<Blocking_canceled>(
					[&] () { //Genode::debug("sending signal RPC");
					         _signal_proxy_cap.call<Signal_proxy::Rpc_signal>();
					         //Genode::debug("signal RPC returned");
					         },
...
Genode::warning("wait_and_dispatch_one_io_signal()");
	...
	Genode::debug("wait_and_dispatch_one_io_signal(): dispatching io signal: ", &sig);
