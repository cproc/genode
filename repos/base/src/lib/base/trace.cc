...
#if 0
Genode::raw(&dummy, ": creating Attached_rom_dataspace");
		try {

			Attached_rom_dataspace config(_env(), "config");
Genode::raw(&dummy, ": created Attached_rom_dataspace");

			bool const sync_trace =
				config.xml().attribute_value("sync_trace", false);

Genode::raw(&dummy, ": Trace::Logger::init(): sync_trace: ", sync_trace);

			if (!sync_trace)
				return;

		} catch (...) {
Genode::raw(&dummy, ": exception");
		}
#else
#endif
