...
//Genode::warning("Vfs_dataspace(): check 2");
			...
			if (root_dir.open(name.string(),
				...
				return;
//Genode::warning("Vfs_dataspace(): check 3");
			...
			for (Vfs::file_size bytes_read = 0; bytes_read < stat_out.size; ) {
				...
				while (!file->fs().queue_read(file, stat_out.size - bytes_read))
					; // XXX block on generic io response lock
				...
				for (;;) {
					read_result = file->fs().complete_read(file, addr + bytes_read,
						                               	   stat_out.size,
						                               	   out_count);
					Genode::warning("Vfs_dataspace(): size: ", stat_out.size,
				                	", out_count: ", out_count);
...
//Genode::warning("Vfs_dataspace(): check 4");
...
//Genode::warning("Vfs_dataspace(): check 5");

			rm.detach(addr);
			root_dir.close(file);
//Genode::warning("Vfs_dataspace(): finished");
		}
	}
...
class Noux::Rom_session_component : public Rpc_object<Rom_session>
{
	...
	private:
		...
		{
			if (name.string()[0] == '/') {
				_rom_from_vfs.construct(_root_dir, name, env.ram(), env.rm(),
				                        _alloc);
