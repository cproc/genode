...
Genode::log("PID ", pid(), ": SYSCALL_PIPE: ", _sysio.pipe_out.fd[0], "/", _sysio.pipe_out.fd[1]);
		...
		case SYSCALL_READLINK:
		{
			...
			Genode::log("SYSCALL_READLINK");

			for (;;) {
				Genode::log("SYSCALL_READLINK: calling queue_readlink()");
				if (_root_dir.queue_readlink(_sysio.readlink_in.path,
				                             min(_sysio.readlink_in.bufsiz,
			                                     sizeof(_sysio.readlink_out.chunk)),
			                                 &context))
			    	break;

				Genode::log("SYSCALL_READLINK: blocking for queue_readlink()");
				context.wait_for_completion();
				Genode::log("SYSCALL_READLINK: unblocked for queue_readlink()");
			}

			for (;;) {
				Genode::log("SYSCALL_READLINK: calling complete_readlink()");

				_sysio.error.readlink =
					_root_dir.complete_readlink(_sysio.readlink_in.path,
				                                _sysio.readlink_out.chunk,
				                                min(_sysio.readlink_in.bufsiz,
			    	                                sizeof(_sysio.readlink_out.chunk)),
			        	                        out_count);

				if (_sysio.error.readlink != Vfs::Directory_service::READLINK_QUEUED)
					break;

				Genode::log("SYSCALL_READLINK: blocking for complete_readlink()");
				context.wait_for_completion();
				Genode::log("SYSCALL_READLINK: unblocked for complete_readlink()");
			}
