...
extern "C" void genode_store_register(int regno, unsigned long reg_content)
{
	...
	switch ((enum reg_index)regno)
	{
		case EAX:  thread_state.eax = reg_content; PDBG("EAX = %8lx", reg_content); return;
		case ECX:  thread_state.ecx = reg_content; PDBG("ECX = %8lx", reg_content); return;
		case EDX:  thread_state.edx = reg_content; PDBG("EDX = %8lx", reg_content); return;
		case EBX:  thread_state.ebx = reg_content; PDBG("EBX = %8lx", reg_content); return;
		case UESP: thread_state.sp = reg_content; PDBG("ESP = %8lx", reg_content); return;
		case EBP:  thread_state.ebp = reg_content; PDBG("EBP = %8lx", reg_content); return;
		case ESI:  thread_state.esi = reg_content; PDBG("ESI = %8lx", reg_content); return;
		case EDI:  thread_state.edi = reg_content; PDBG("EDI = %8lx", reg_content); return;
		case EIP:  thread_state.ip = reg_content; PDBG("EIP = %8lx", reg_content); return;
		case EFL:  thread_state.eflags = reg_content; PDBG("EFL = %8lx", reg_content); return;
