...
int Usb_proxy_device_genode::urb_queue(PUSBPROXYDEV pProxyDev, PVUSBURB pUrb)
{
	...
	try {
		...
		if (pUrb->enmType == VUSBXFERTYPE_MSG) { 
			...
			PDBG("control %s transfer: request_type = 0x%x, request = 0x%x, value = 0x%x, index = 0x%x, length = %u, cbData = %u",
			 	 dir_str, pSetup->bmRequestType, pSetup->bRequest, pSetup->wValue, pSetup->wIndex, pSetup->wLength, pUrb->cbData);
			...
			if ((pUrb->enmDir == VUSBDIRECTION_OUT) && (pSetup->wLength > 0)) {
				...
				PDBG("copying %zu bytes", pSetup->wLength);
			...
			if ((pUrb->enmDir == VUSBDIRECTION_OUT) && (pUrb->cbData > 0)) {
				char *packet_content = data->usb_connection.source()->packet_content(p);
				PDBG("copying %zu bytes", pUrb->cbData);
	...
	if (!data->usb_connection.source()->ack_avail()) {
		...
		for (;;) {
			...
			if (dynamic_cast<Timeout_signal_context*>(context)) {
				...
				//pProxyDev->fDetached = !data->usb_connection.plugged();
				return 0;

			} else {

				PDBG("unhandled signal received");
				continue;
