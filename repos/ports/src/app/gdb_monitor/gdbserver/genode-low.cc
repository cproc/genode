...
PDBG("thread: %p", thread);
				...
				if (signal == SIGSTOP) {
					...
					/*
					 * First signal of a new thread. On Genode a SIGTRAP, but
					 * gdbserver expects SIGSTOP.
					 */
...
extern "C" long ptrace(enum __ptrace_request request, pid_t pid, void *addr, void *data)
{
	...
	switch (request) {
		...
		case PTRACE_GETEVENTMSG:
			 ...
			 */
			read(genode_new_thread_pipe_read_fd(), data, sizeof(unsigned long));
			return 0;
		case PTRACE_GETREGSET: request_str = "PTRACE_GET_REGSET";  break;
	}

	PDBG("ptrace(%s (0x%x)) called - not implemented!", request_str, request);
...
PDBG("writing new lwpid: %lu", lwpid);
	...
	if (myaddr && (len > 0)) {
		if (debug_threads) {
			...
			if (len == 1)
				...
				val = val & 0xffffff;
			PDBG("Writing %08x to 0x%08lx", 2 * ((len < 4) ? len : 4),
			     val, (long)memaddr);
		}
