...
void genode_add_thread(unsigned long lwpid)
{
	if (lwpid == GENODE_LWP_BASE) {
		...
		/* unblock 'my_waitpid()', so it can listen for signals of the new thread, too */
		genode_send_signal_to_thread(GENODE_LWP_BASE, SIGINFO);
	}
}
...
int genode_send_signal_to_thread(unsigned long lwpid, int signo)
{
	Cpu_session_component *csc = gdb_stub_thread()->cpu_session_component();
	...
	csc->deliver_signal(thread_cap, signo);
