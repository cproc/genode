...
LDFLAGS += $(LD_OPT_NOSTDLIB)

# from repos/base/mk/global.mk
LINK_SPEC_COMMON = $(LD_OPT)

# from repos/base/mk/prg.mk
LINK_SPEC_NONSHARED = -Ttext=$(LD_TEXT_ADDR) \
                      -T$(LD_SCRIPT_DYN) \
                      -rpath-link=$(PWD) \
                      --dynamic-linker=$(DYNAMIC_LINKER).lib.so \
                      --eh-frame-hdr

# from repos/base/mk/so.mk
LINK_SPEC_SHARED = -shared \
                   -T$(LD_SCRIPT_SO) \
                   --entry=0x0 \
                   --whole-archive -l:ldso_so_support.lib.a --no-whole-archive

LDFLAGS += -specs=$(PWD)/link_spec
...
#
Makefile reconfigure: env.sh link_spec static_libs_symlinks.tag $(SHARED_LIBS)
	@$(MSG_CONFIG)$(TARGET)
...
link_spec:
	$(VERBOSE)rm -f $@
	$(VERBOSE)echo "*link:" >> $@
	$(VERBOSE)echo "$(LINK_SPEC_COMMON) \\" >> $@
	$(VERBOSE)echo "%{!shared: $(LINK_SPEC_NONSHARED)} \\" >> $@
	$(VERBOSE)echo "%{shared: $(LINK_SPEC_SHARED)}" >> $@
