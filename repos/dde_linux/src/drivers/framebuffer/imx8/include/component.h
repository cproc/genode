...
class Framebuffer::Session_component : public Genode::Rpc_object<Session>
{
	...
	public:
		...
		Mode mode() const override {
			Genode::error("mode() called, not implemented");
			return Mode(1920/*_driver.width()*/, 1080/*_driver.height()*/, Mode::RGB565); }

		void mode_sigh(Genode::Signal_context_capability sigh) override {
			Genode::error("mode_sigh() called, not implemented");
			/*_mode_sigh = sigh;*/ }

		void sync_sigh(Genode::Signal_context_capability sigh) override
		{
			Genode::error("sync_sigh() called, not implemented");
#if 0
			_timer.sigh(sigh);
		...
		{
			Genode::error("refresh() called, not implemented");
#if 0
