...
{
	lx_printf("of_property_read_bool(): device: %s, property: %s\n", np->name, propname);
	...
	if ((Genode::strcmp(np->name, "hdmi", strlen(np->name)) == 0)) {
		...
		Genode::error(__func__, "(): unhandled property\n");

		return false;
	}

	Genode::error(__func__, "(): unhandled device\n");
...
{
	lx_printf("of_property_read_string(): device: %s, property: %s\n",
	          np->name, propname);
	
	if ((Genode::strcmp(np->name, "hdmi", strlen(np->name)) == 0) &&
	    (Genode::strcmp(propname, "compatible") == 0)) {
		*out_string = "fsl,imx8mq-hdmi";
		return 0;
	}

	Genode::error(__func__, "(): unhandled device or property");
...
{
	lx_printf("of_property_read_u32(): device: %s, property: %s\n",
	          np->name, propname);

	if ((Genode::strcmp(np->name, "hdmi", strlen(np->name)) == 0) &&
	    (Genode::strcmp(propname, "hdcp-config") == 0)) {
...
#endif
Genode::log("_preferred_mode(): ", (connector->status == connector_status_connected));
		/**
