...
struct device_node *of_graph_get_next_endpoint(const struct device_node *parent,
                                               struct device_node *prev)
{
	lx_printf("*** of_graph_get_next_endpoint(): parent: %s, prev: %s, ret: %p\n",
	          parent->name, prev ? prev->name : "", __builtin_return_address(0));
	...
	if (Genode::strcmp(parent->name, "hdmi", strlen(parent->name)) == 0) {
		if (!prev) {
			lx_printf("of_graph_get_next_endpoint(): returning endpoint_device_node\n");
