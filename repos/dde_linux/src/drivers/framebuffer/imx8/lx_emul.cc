...
struct device_node *of_get_parent(const struct device_node *node)
{
	...
	if (Genode::strcmp(node->name, "dcss_disp0", strlen("dcss_disp0")) == 0)
...
struct device_node *of_parse_phandle(const struct device_node *np, const char *phandle_name, int index)
{
	...
	/* information from fsl-imx8mq.dtsi */
	static device_node ports_device_node { "dcss_disp0", "dcss_disp0" };

	if (Genode::strcmp(phandle_name, "ports", strlen(phandle_name)) == 0)
		return &ports_device_node;

	Genode::error("of_parse_phandle(): unhandled phandle");
...
lx_printf("of_graph_get_port_by_id(): parent: %p, ret: %p\n", parent, __builtin_return_address(0));
	...
	static struct device_node port0_device_node { "dummy_port0", "dummy_port0" };
