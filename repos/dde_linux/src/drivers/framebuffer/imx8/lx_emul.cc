                                       ...
                                       unsigned int type, unsigned int num)
{
	...
	static struct resource dcss_mem = { 0x32e00000, 0x32effff, "dcss", 0 };
...
{
lx_printf("of_get_property(): %s, %p\n", name, __builtin_return_address(0));

	if (Genode::strcmp(name, "disp-dev", strlen(name)) == 0)
		return "hdmi_disp";

	Genode::error("of_get_property(): unhandled property");
...
{
	lx_printf("of_match_device(): dev: %s, matches: %p, matches->compatible: %s\n",
	          dev->name, matches, matches->compatible);

	lx_printf("of_match_device: for: %u\n", matches && matches->compatible);

	for (; matches && matches->compatible; matches++)
		lx_printf("of_match_device(): match: %s\n", matches->compatible);

	//if (Genode::strcmp(dev->name, "i.mx8-hdp", strlen(dev->name)) == 0)
	return NULL;
}
...
{
lx_printf("of_graph_get_port_by_id(): parent: %p, ret: %p\n", parent, __builtin_return_address(0));

	static struct device_node port0_device_node {
		.name = "dummy_port0",
		.full_name = "dummy_port0",
		.parent = nullptr
	};

	if (id == 0)
		return &port0_device_node;

	Genode::error("of_graph_get_port_by_id(): unhandled id\n");
...
{
	lx_printf("drm_of_component_probe_with_match()\n");
	return m_ops->bind(dev);
}
#endif
...
void *devm_ioremap(struct device *dev, resource_size_t offset,
                   unsigned long size)
{
lx_printf("devm_ioremap(): offset: %x, size: 0x%lx, ret: %p\n", offset, size, __builtin_return_address(0));
...
{
	return Lx::Malloc::dma().alloc(size, PAGE_SHIFT, dma_addr);
}
