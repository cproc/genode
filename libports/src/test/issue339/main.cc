...
void FstreamTest::File_Open(string filename)
{
	stat_file.open( filename.c_str(), ios::in | ios::out | ios::trunc );

	while( !stat_file.is_open() )
	{
		timer.msleep(4000);
		stat_file.open( filename.c_str(), ios::in | ios::out | ios::trunc );
	}
...
int server()
{
	...
	in_addr.sin_addr.s_addr = inet_addr("127.0.0.1");   //INADDR_ANY;   
	...
	PDBG("File Open : fstream.txt\n");

	FstreamTest file;
	file.File_Open("/fstream.txt");
