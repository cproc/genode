...
void wait_for_continue();
	...
	struct pthread : Thread<STACK_SIZE>
	{
		...
		void entry()
		{
			Thread_base *_myself = myself();
			PDBG("myself = %p, &_myself = %p", _myself, &_myself);
			PDBG("stack_top = %p, stack_base = %p", _myself->stack_top(), _myself->stack_base());
	...
	pthread_t pthread_self(void)
	{
		...
		PDBG("pthread = %p, &pthread = %p", pthread, &pthread);
	                          ...
	                          size_t *stacksize)
	{
		...
		*stacksize = (addr_t)pthread->stack_top() - (addr_t)pthread->stack_base();
		...
		Thread_base *myself = Thread_base::myself();
		PDBG("myself = %p", myself);
		PDBG("myself: stack_top = %p, stack_base = %p", myself->stack_top(), myself->stack_base());

		PDBG("stack_top = %p, stack_base = %p, stacksize = %zu", pthread->stack_top(), pthread->stack_base(), *stacksize);
		wait_for_continue();
