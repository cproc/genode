...
PDBG("adding range %zx - %zx", (addr_t)local_addr, (addr_t)local_addr + size - 1);
...
PDBG("ds_addr = %p, sizeof(Dataspace) = %zu", ds_addr, sizeof(Dataspace));
...
void *Libc::Mem_alloc_impl::alloc(size_t size, size_t align_log2)
{
	 ...
	 */
	size_t request_size = size + (1 << align_log2);
...
PDBG("size = %zu, align_log2 = %zu", size, align_log2);
