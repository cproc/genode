 ...
 */
__attribute__((weak)) void stat_cache_insert(const char *path, int result, int errno_, struct stat const &buf) { }

__attribute__((weak)) int stat_cache_lookup(const char *path, struct stat *buf)
{
	...
	do {
		...
		while (t) {
			...
			if (!symlink_resolved_in_this_iteration) {
				...
				int res = stat_cache_lookup(next_iteration_working_path.base(), &stat_buf);
				if (res == -2) {
					FNAME_FUNC_WRAPPER_GENERIC(res = , stat, next_iteration_working_path.base(), &stat_buf);
					stat_cache_insert(next_iteration_working_path.base(), res, errno, stat_buf);
				}
				if (res == -1) {
...
extern "C" int lstat(const char *path, struct stat *buf)
{
	try {
		...
		int res = stat_cache_lookup(resolved_path.base(), buf);
		if (res == -2) {
			FNAME_FUNC_WRAPPER_GENERIC(res = , stat, resolved_path.base(), buf);
			stat_cache_insert(resolved_path.base(), res, errno, *buf);
		}
		return res;
	} catch (Symlink_resolve_error) {
		return -1;
