...
namespace Noux {
	...
	struct Sysio
	{
		...
		union {
			...
			SYSIO_DECL(socket, { int domain; int type; int protocol; }, { int fd; });
			...
			SYSIO_DECL(setsockopt, { int fd; int level; int optname; Chunk optval;
					socklen_t optlen; }, { });
			...
			SYSIO_DECL(listen, { int fd; int type; int backlog; }, { int result; });
