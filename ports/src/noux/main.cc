...
bool Noux::Child::syscall(Noux::Session::Syscall sc)
{
	...
	try {
		...
		case SYSCALL_FSTAT:
			//PDBG("%d: SYSCALL_FSTAT: fd = %d", pid(), _sysio->fstat_in.fd);
			return _lookup_channel(_sysio->fstat_in.fd)->fstat(_sysio);

		case SYSCALL_FCNTL:

			//PDBG("%d: SYSCALL_FCNTL: fd = %d, cmd = %d", pid(), _sysio->fcntl_in.fd, _sysio->fcntl_in.cmd);
		...
		case SYSCALL_OPEN:
			{
				...
				//PDBG("%d: SYSCALL_OPEN: fd = %d", pid(), _sysio->open_out.fd);
				return true;
			}
		...
		case SYSCALL_LSEEK:
			//PDBG("%d: SYSCALL_LSEEK: fd = %d", pid(), _sysio->lseek_in.fd);
