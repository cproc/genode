...
namespace Noux {
	              ...
	              public Interrupt_handler
	{
		private:
			 ...
			 * 
			 * The only reason to have it as a member variable instead of
			 * creating it on demand is to not have to register and unregister
			 * it as interrupt handler at every IO channel on every blocking
			 * syscall, but only once when the IO channel gets added.
			 */
			Lock _blocker;
		...
		public:
			...
			{
PDBG("%d: add_io_channel(%d)", pid(), fd);
				fd = File_descriptor_registry::add_io_channel(io_channel, fd);
PDBG("%d: add_io_channel(): new fd = %d", pid(), fd);
			...
			{
				PDBG("%d: remove_io_channel(%d)", pid(), fd);
		...
		PDBG("%d: calling _blocker.unlock()", pid());
				_blocker.unlock();
			}
