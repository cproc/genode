...
namespace Noux {
	...
	class Socket_io_channel : public Io_channel
	{
		...
		public:
			...
			{
				/**
				 * For now this fcntl() only contains stubs to keep
				 * programs happy. lwip-1.3.2 which is currently used
				 * does not provide its own lwip_fcntl(). We could only
				 * use lwip_ioctl() to set the socket O_NONBLOCKING if
				 * we get this kind of request.
				 */
			...
			{
				_socket = ::socket(sysio->socket_in.domain, sysio->socket_in.type,
			...
			{
				int result = ::getsockopt(_socket, sysio->getsockopt_in.level, sysio->getsockopt_in.optname,
							  sysio->getsockopt_in.optval, &sysio->getsockopt_in.optlen);
			...
			{
				/**
				 * Filter options out because lwip only supports several socket options.
				 * Therefore for now we silently return 0 and notify the user via debug
				 * message.
				 */
				...
				case SO_REUSEADDR:
					{
					PWRN("SOL_SOCKET option '%d' is currently not supported, however we report success", sysio->setsockopt_in.optname);
					return true;
					}
				}

				int result = ::setsockopt(_socket, sysio->setsockopt_in.level, sysio->setsockopt_in.optname,
							  sysio->setsockopt_in.optval, sysio->setsockopt_in.optlen);
