...
extern "C" void genode_store_register(int regno, unsigned long reg_content)
{
	...
	switch((enum reg_index)regno)
	{
		...
		case CS:   PDBG("cannot change CS register"); PDBG(" CS = %8lx", reg_content); break;
		case SS:   PDBG("cannot change SS register"); PDBG(" SS = %8lx", reg_content); break;
		case DS:   PDBG("cannot change DS register"); PDBG(" DS = %8lx", reg_content); break;
		case ES:   PDBG("cannot change ES register"); PDBG(" ES = %8lx", reg_content); break;
