	...
	public:
		...
		void tx(typename TX_QUEUE::Packet_descriptor packet, bool inhibit_wakeup)
		{
			...
			do {
				...
				if (_tx_queue->full()) {
					/*
					 * FIXME: it might be better not to call 'wakeup_rx()' here,
					 *        because it could cover up a forgotten manual
					 *        'wakup_rx()' call in the case that the queue does not
					 *        get full.
					 */
					wakeup_rx();
...
class Packet_descriptor_receiver
{
	...
	public:
		...
		void rx(typename RX_QUEUE::Packet_descriptor *out_packet, bool inhibit_wakeup = false)
		{
			...
			/* block for signal if rx queue is empty */
			while (_rx_queue->empty()) {
				/*
				 * FIXME: it might be better not to call 'wakeup_tx()' here,
				 *        because it could cover up a forgotten manual
				 *        'wakup_tx()' call in the case that the queue does not
				 *        get empty.
				 */
				wakeup_tx();
