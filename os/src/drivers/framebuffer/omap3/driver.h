...
class Framebuffer::Driver
{
	...
	private:
		...
		/* memory map */
		enum {
			DSS_MMIO_BASE   = 0x48050000,
			DSS_MMIO_SIZE   = 0x00000200,

			DISPC_MMIO_BASE = 0x48050400,
			DISPC_MMIO_SIZE = 0x0400,
#if 0
...
:
	...
	_dss((addr_t)_dss_mmio.local_addr<void>() | (DSS_MMIO_BASE & 0xfff)),

	_dispc_mmio(DISPC_MMIO_BASE, DISPC_MMIO_SIZE),
	_dispc((addr_t)_dispc_mmio.local_addr<void>() | (DISPC_MMIO_BASE & 0xfff))//,
#if 0
...
{
	/* set logic clock divisor to 1 and pixel clock divisor to 6 */
	_dispc.write<Dispc::Divisor::Lcd>(1);
	_dispc.write<Dispc::Divisor::Pcd>(6);
	...
	_dispc.write<Dispc::Gfx_attributes::Channelout>(Dispc::Gfx_attributes::Channelout::LCD);
#if 0
	...
	if (!_dispc.wait_for<Dispc::Control1::Go_lcd>(Dispc::Control1::Go_lcd::HW_UPDATE_DONE, _delayer)) {
