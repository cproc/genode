...
class Rom_session_component : public Genode::Rpc_object<Genode::Rom_session>
{
	private:
		                                                  ...
		                                                  Path const &path)
		{
			...
			PERR("could not open compound directory for observing changes");
		                                           ...
		                                           Path const &path)
		{
			...
			try {
				...
				Dir_handle dir = _open_compound_dir(fs, path);
		...
		void _update_dataspace()
		{
			...
			if (!_file_ds.valid()) {
				...
				_compound_dir_handle = _open_compound_dir(_fs, _file_path);
	...
	public:
		...
		{
			if (!_file_handle.valid()) {
				PWRN("lookup for file \"%s\" failed", file_path);
